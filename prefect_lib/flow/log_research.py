...
with Flow(
    name='Log research flow',
    state_handlers=[status_change],
) as flow:

    log_type = Parameter('log_type', required=False)()
    ...
    result = task(domain=domain,
                  crawling_start_time_from=start_time_from,
                  crawling_start_time_to=start_time_to,
                  urls=urls)

# domain、crawling_start_time_*による絞り込みは任意
flow.run(parameters=dict(
    log_research='',    #scrapy_stats,resource_check,log_level(CRITICAL,ERROR,WARNING),error_url,difference_record
    domain='kyodo.co.jp',
